...
function linux_product_export_func(){
	...
	<div class="wrap">
		...
		if(!empty($datas)){
			...
			/* <![CDATA[ */
			var csv_data = <?php echo json_encode($datas)?>;
			/* ]]> */
			(function($){
				$(document).ready(function(){
					...
					function ajax_each(index, value){
						...
						$.ajax({
							...
							data : {
								action: "creat_post_by_data", 
								data_row_csv : value[index]
							},
							context: this,
							beforeSend: function(){
								if(index == 0){
									$('.mess_process').html('Loading ...<br>');
								}
							}
							...
							setTimeout(function(){
								$('.mess_process').prepend('Reload '+ index +'...<br>');
								ajax_each(index,csv_data);
							}, 3000);
						});
					}
					ajax_each(0,csv_data);			
				});
			})(jQuery)
			</script>			
			<?php
...
add_action('wp_ajax_creat_post_by_data', 'creat_post_by_data_func');
function creat_post_by_data_func(){
	$data = $_POST['data_row_csv'];
	if(empty($data) || !is_array($data)) wp_send_json_error();	
	if(count($data) == 7){
		$category = ($data[0])?$data[0]:'';
		$manufacturer = ($data[1])?$data[1]:'';
		$type = ($data[2])?$data[2]:'';
					...
					if(!empty($TestSignal_cat) && is_array($TestSignal_cat) && !is_wp_error($TestSignal_cat)){
						if(!check_posts_exits(array($category_parent['term_id'],$category_child['term_id'],$type,$DeviceID,$Firmware))){
							...
							if($product_id){
								...
								wp_send_json_success( implode('|', $data) .' ==> <span style="color: #4CAF50;">ADD Okie</span> <br>');
							}else{															
								wp_send_json_success( implode('|', $data) .' ==> <span style="color: red;">Add Post Error</span> <br>');
							}
						}else{
							wp_send_json_success( implode('|', $data) .' ==> <span style="color: red;">Post Exits</span> <br>');
						}
					}else{
						wp_send_json_success( implode('|', $data) .' ==> <span style="color: red;">Status Error</span> <br>');
					}
				}else{
					wp_send_json_success( implode('|', $data) .' ==> <span style="color: red;">Manufacturer Error</span> <br>');
				}
			}else{
				wp_send_json_success( implode('|', $data) .' ==> <span style="color: red;">Category Error</span> <br>');
			}								        	
		}else{
			wp_send_json_success(implode('|', $data) .' ==> <span style="color: red;">Category && manufacturer Not exits</span> <br>');
		}
	}
	wp_send_json_error();
}
