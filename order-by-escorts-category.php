...
function get_all_not_escorts(){
    $all_prods = array_reverse(devvn_get_all_post_by_type_escorts());
    $new_args = array();
    if($all_prods && !empty($all_prods)) {
        foreach ($all_prods as $escorts_post) {
            foreach ($escorts_post as $postid) {
                $new_args[] = $postid;
            }
        }
    }
...
function show_all_prod( $query ) {
    if ( !is_admin()  && is_woocommerce() && (is_shop() || is_product_category() || is_product_tag()) && $query->is_main_query() ) {
        $query->set( 'posts_per_page', -1 );
        $query->set('post__in', get_all_not_escorts());
    }
}
